...
use crate::services::layers::persisted_queries::ManifestOperation;
pub use crate::services::layers::persisted_queries::{
    FullPersistedQueryOperationId, PersistedQueryManifest,
};
...
/// Get a query ID, body, and a PQ manifest with that ID and body.
pub fn mock_manifest(operations: Vec<(String, String, Option<String>)>) -> PersistedQueryManifest {
    PersistedQueryManifest::from(
        operations
            .into_iter()
            .map(|(id, body, client_name)| ManifestOperation {
                id,
                body,
                client_name,
            })
